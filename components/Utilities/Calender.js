...
export default function UserCalender(props) {
  ...
  const RenderCalender = () => {
    const endDate = moment
      .utc(userdetail.startDate)
      .add(5, 'day')
      .format();
    let range = calenderFun.getDaysDiff(
      moment(userdetail.endDate),
      moment(userdetail.startDate),
    );
    let nextDays = [];
    if (range > 0) {
    ...
    nextDays.forEach(date => {
      ...
      if (index === 0) {
        (strdy = true), (enddy = false);
      } else if (index === 6) {
        (strdy = false), (enddy = true);
      } else {
        (strdy = false), (enddy = false);
      }
      markDate = {
        ...markDate,
        [date]: {
  ...
  return (
    <View style={styles.dateIconContainer}>
      <Text style={styles.dateIcon} onPress={handleCalender}>
        {displayDate}{' '}
        <Icon
          name="calendar"
          type="font-awesome"
